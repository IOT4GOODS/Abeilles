...
void loop()
{
 if(count==N) 
 {
  ...
  scales = scalesum/N;
  temp = tempsum/N;
  poids_g_hex=ulongtoHex((unsigned long)(scales*1000)); // temperature non compense en gramme
  sensors.requestTemperatures(); 
  RemoteTemp=sensors.getTempCByIndex(0);
  RTemps_hex = uinttoHex((short)(RemoteTemp*100));
  LTemps_hex = uinttoHex((short)(temp*100));
  msg = (String)ATSIGFOXTX + poids_g_hex + LTemps_hex + RTemps_hex; // Put everything together
